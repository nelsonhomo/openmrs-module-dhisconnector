...
function buildDXFJSON(locationUid, orgUnitId, serverUuid) {
    ...
    return getReportData(locationUid).then(function () {
		let mappingNamePlusDateCreated = 'Generated by DHIS Connector OpenMRS Module. <=> '+selectedMapping.name +' <=> '+formatDate(new Date(selectedMapping.created));
        ...
        for(let i =0; i < reportData.dataSets.length; i++){
            ...
            if (indicatorValues.hasOwnProperty(indicator)) {
                ...
                if (mapping !== null) {
                    ...
                    dataValue.comment = mappingNamePlusDateCreated
